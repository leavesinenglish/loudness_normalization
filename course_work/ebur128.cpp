...
using filter_state = double[FILTER_STATE_SIZE];
...
static int ebur128_init_channel_map(ebur128_state* st) {
	...
	if (st->channels == 4) {
		...
		for (i = 0; i < st->channels; ++i) {
			switch (i) {
			case 0: st->d->channel_map[i] = EBUR128_LEFT;
				break;
			case 1: st->d->channel_map[i] = EBUR128_RIGHT;
				break;
			case 2: st->d->channel_map[i] = EBUR128_CENTER;
				break;
			case 3: st->d->channel_map[i] = EBUR128_UNUSED;
				break;
			case 4: st->d->channel_map[i] = EBUR128_LEFT_SURROUND;
				break;
			case 5: st->d->channel_map[i] = EBUR128_RIGHT_SURROUND;
				break;
			default: st->d->channel_map[i] = EBUR128_UNUSED;
				break;
			}
		}
	}
...
int ebur128_set_max_window(ebur128_state* st, unsigned long window) {
	...
	auto new_audio_data = (double*)malloc(new_audio_data_size);
...
static int ebur128_double_cmp(const void* p1, const void* p2) {
	auto d1 = (const double*)p1;
	auto d2 = (const double*)p2;
	return (*d1 > *d2) - (*d1 < *d2);
}
