...
int CReadWaveFile::ReadSamples(short int* buffer, int max_sample_count) {
	if (m_nSampleCount <= 0)
		return 0;
	int cnt = min(m_nSampleCount, max_sample_count);
...
bool CWriteWaveFile::WriteSamples(short int* buffer, int sample_count) {
	if (m_nSampleCnt < 0)
		return false;
	int size = mmioWrite(m_hFile, (HPSTR)buffer, sample_count * 4);
	if (size < 0)
		return false;
	// all right
	m_nSampleCnt += size / 4;
